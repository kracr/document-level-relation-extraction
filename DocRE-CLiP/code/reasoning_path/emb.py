...
class EmbeddingBasedMethod(LFramework):
    ...
    def loss(self, mini_batch):
        ...
        r_label = ((1 - self.label_smoothing_epsilon) * r) + (1.0 / r.view(-1,1))
        pred_scores = mdl.forward(e1,e2,r, kg)
        loss = self.loss_fun(pred_scores, r_label)
        loss_dict = {}
    ...
    def predict(self, mini_batch, verbose=False):
        ...
        if self.model == 'hypere':
            pred_scores = mdl.forward(e1, e2, kg, [self.secondary_kg])
        elif self.model == 'triplee':
            pred_scores = mdl.forward(e1, e2, kg, [self.secondary_kg, self.tertiary_kg])
        else:
            pred_scores = mdl.forward(e1, e2, kg)
        return pred_scores
