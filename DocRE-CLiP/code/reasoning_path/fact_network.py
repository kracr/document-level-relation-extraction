...
class ConvE(nn.Module):
    ...
    def forward(self, e1,e2,r,kg):
        # Reshape E2 to match thei shape of E1
        e2=e2.long()
        E1 = kg.get_entity_embeddings(e1).view(-1, 1, self.emb_2D_d1, self.emb_2D_d2)
        E2 = kg.get_entity_embeddings(e2).view(-1, 1, self.emb_2D_d1, self.emb_2D_d2)
        R = kg.get_all_relation_embeddings()
        E2 = torch.tensor(E2).to(torch.int64)
        stacked_inputs = torch.cat([E1, E2], 2)
    ...
    def forward_fact(self, e1, r, e2, kg):
        ...
        E1 = kg.get_entity_embeddings(e1).view(-1, 1, self.emb_2D_d1, self.emb_2D_d2)
        E2 = kg.get_relation_embeddings(e2).view(-1, 1, self.emb_2D_d1, self.emb_2D_d2)
        R = kg.get_entity_embeddings(r)
