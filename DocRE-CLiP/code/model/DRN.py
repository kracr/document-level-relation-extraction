...
class DRN_GloVe(nn.Module):
    ...
    def forward(self, **params):
        ...
        for i in range(len(params["titles"])):
                    j = 0
                    for h_idx in range(params["L_vertex"][i]):
                        for t_idx in range(params["L_vertex"][i]):
                            if h_idx == t_idx:
                                continue
                            for label_idx in range(1,relation_num):
                                score_value = scores[i, j, label_idx]
                                score_tensor = torch.tensor(score_value)
                                score_tensor=score_tensor+0.01
                                j += 1
        return {"predictions":scores,"task_select":task_select}



def getvalue(idx,title):

 with open('/home/DRN/code/models/keyvalue.txt',"r") as fg:
    #sub="hello"
    #obj="hello"
    for line in fg:
        titl,keyid,value=line.split("\t")
       
        if(str(title==str(titl))):
            if(str(idx)==str(keyid)):
                sub=value
                return sub
...
def getrid(docred_id):
  rel2id = json.load(open(('/home/DRN/data/docred/rel2id.json'), "r"))
  id2rel = {v: k for k, v in rel2id.items()}
  #print(id2rel[1])
  relinfo = json.load(open(('/home/DRN/data/docred/rel_info.json'), "r"))
  #print(relinfo[id2rel[1]])

  #remove space
  with open('/home/DRN/code/RGCN/data/docred/relations.dict') as f:
        relation2id = dict()

        for line in f:
            rid, relation = line.strip().split('\t')
            if(str(relation)==str(relinfo[id2rel[1]])):
                #print(rid)
                return rid
...
class DRN_BERT(nn.Module):
    ...
    def forward(self, **params):
        ...
        max_node = torch.max(graph_node_num).item()
        graph_adj = params['graph_adj'][:,:max_node,:max_node]
        #print("ENCODER OUTPUTS: ",encoder_outputs.shape[-1]) 
        ...
        if(params["modeltype"]=="test"):
         for i in range(len(params["titles"])):
                    ...
                    for h_idx in range(params["L_vertex"][i]):
                        for t_idx in range(params["L_vertex"][i]):
                            if h_idx == t_idx:
                                continue
                            for label_idx in range(1,97):
                                        sub=getvalue(h_idx,params["titles"][i])
                                
  
                                        subembed=params["entity_embedding"]
                                        relembed=params["relation_embedding"]
                                        entdict=(params["ent_dict"])
                                        newsub=(sub.strip("\n"))
                                        s = subembed[entdict[newsub]]
                                        #r = w[test_triplets[]
                                        obj=getvalue(t_idx,params["titles"][i])
                                        newobj=(obj.strip("\n"))
                                        o = subembed[entdict[newobj]]
                                        sc = torch.sum(s * o)

                                        scores = predictions.data.cpu().numpy()
                                        scores[i, j, label_idx] = scores[i,j,label_idx]+2.5*sc
                                        scores_tensor = torch.tensor(scores)
                                        scores_tensor = scores_tensor.cuda()

                                #print("current:",scores[i, j, label_idx])
                                #score_tensor = torch.tensor(score_value)
         ...
         return {"predictions":scores_tensor,"task_select":task_select}
        else:
