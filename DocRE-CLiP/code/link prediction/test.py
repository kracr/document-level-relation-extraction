...
def valid(valid_triplets, model, test_graph, all_triplets):
    ...
    mrr = calc_mrr(0,entity_embedding, model.relation_embedding, valid_triplets, all_triplets, hits=[1, 3, 10])
    #mrr=""
    return mrr
...
def test(test_triplets, model, test_graph, all_triplets):
    ...
    entity_embedding = model(test_graph.entity, test_graph.edge_index, test_graph.edge_type, test_graph.edge_norm)
    #l = model.save_score(entity_embedding, train_data.samples, train_data.labels) + reg_ratio * model.reg_loss(entity_embedding)
    mrr = calc_mrr(1,entity_embedding, model.relation_embedding, test_triplets, all_triplets, hits=[1, 3, 10])
...
def main(args):
    ...
    entity2id, relation2id, train_triplets, valid_triplets, test_triplets = load_data('./data/docred')
    all_triplets = torch.LongTensor(np.concatenate((train_triplets, valid_triplets, test_triplets)))
    print("all triples lenth:",len(all_triplets))
